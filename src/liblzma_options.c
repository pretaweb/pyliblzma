...
level\t lc\t lp\t pb\t mode\t mf\t nice_len\t depth\t dict_size\n");
	...
	for(levelNum = LZMA_BEST_COMPRESSION; levelNum >= LZMA_BEST_SPEED; levelNum--){
        ...
        options_dict = LZMA_options_get(filter);
#if PY_VERSION_HEX >= 0x020400F0
		settingsString = PyString_Format(levelopts, PyTuple_Pack(9,
#else
		settingsString = PyString_Format(levelopts, Py_BuildValue("(OOOOOOOOO)",
#endif
...
"Nice length of a match (also known as number of fast bytes) (%u - %u)\n\
...
'%s': SHA-256.\n");
	...
	LZMAOptions_members[11] = memberDef(NULL, 0, 0, 0, NULL);	/* Sentinel */
