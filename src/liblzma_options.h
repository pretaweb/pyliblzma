...
static inline char* tostring(const char* text, PyObject *variable) {
	PyObject *format = PyString_FromString(text);
	PyObject *str = PyString_Format(format, variable);
...
static inline PyMemberDef memberDef(char *name, int type, Py_ssize_t offset, int flags, char *doc) {
	PyMemberDef tmp;
	tmp.name = name, tmp.type = type, tmp.offset =offset, tmp.flags = flags, tmp.doc = doc;
	return tmp;
}

#define MEMBER_DESCRIPTOR(name, type, variable, text) memberDef(name, type, offsetof(LZMAOptionsObject, variable), RO, tostring(text, self->variable))
